...
> ⚠️ **2026-07-19 bug-fix passes — read before trusting "done" status:** Two full-repo bug-hunt passes ran this date (see `ConfirmedBugs.md`, commits `f0a2173` and `db9fcd6`), fixing 16 real bugs total (11 in pass 1, 5 in pass 2) including several UI dashboard buttons that silently 403'd and multiple `setTimeout`-based schedulers that don't survive this app's own self-triggered redeploys. A third pass applied CodeRabbit's PR review findings (see PR #33) — 7 more real issues fixed, including a genuine BullMQ jobId-reuse bug in pass 2's own sprint-continuation fix that would have silently broken multi-task sprints after the first task. All fixes are backed by `tsc --noEmit` (clean) and mocked unit tests — **none of it was verified by actually running the backend against real Postgres/Redis or by clicking through the UI in a browser**, except for one manual local boot-and-click-through session (also 2026-07-19) that exercised the UI proxy fix specifically against a live (dummy-credentialed) backend. Treat "fixed" in that document as "fixed at the code/type level plus targeted manual smoke-testing," not "fully verified against production infrastructure," and re-check before relying on it for anything user-facing.
...
| 6: Architecture Refactoring | **IN PROGRESS** 🏗️ | ~50% (6.1–6.4 done; 6.3 deferred D-005; 6.5 pending) | Needs Phase 4 + 5 |
