...
**PR #58 review follow-up (2026-07-31)**: CodeRabbit + Qodo flagged 4 real issues on the migrated code, all fixed same-day:
1. `callAnyProvider()` accepted an **empty string** as a successful response instead of treating it as a failure — the fallback loop stopped on the first provider that returned no content instead of trying the next one. Directly relevant since this same-day migration widened `callAnyProvider`'s usage to 3 more callers (audit, OWASP, agent personality). Fixed: both the OpenAI-compatible and Anthropic branches now throw `'empty response'` on empty content, which the existing per-provider `catch` already handles.
2. `callAnthropic()` silently dropped `opts.temperature` and `opts.timeoutMs` — Anthropic calls ignored both. Fixed: `temperature` is now included in the request body when set, and `timeoutMs` is passed via the SDK's per-request `{ timeout }` option.
3. `ceoReport.callAI()` didn't pass `timeoutMs` to `callAnyProvider()`, silently falling back to the client's 30s default — a regression from the pre-migration hand-rolled `axios.post` calls, which used a 60s timeout. Fixed: `timeoutMs: 60000` restored explicitly.
4. Qodo flagged that `callAnyProvider()` fires billable third-party API calls purely off key presence, with no explicit opt-in/kill-switch. Added `AI_PROVIDER_CALLS_ENABLED` (`.env.example`, defaults to enabled so no existing deployment needs a config change to keep working) — set to `false`/`0` to hard-disable all provider calls even when keys are configured (e.g. a staging env sharing prod keys).
New tests added to `test/ai.client.test.ts`: empty-response-triggers-fallback, all-empty-throws, kill-switch-blocks-call, and an Anthropic-path test asserting `temperature`/`timeout` are forwarded (closes the "Anthropic branch untested" gap CodeRabbit also flagged). Also restored `ORIGINAL_ENV`/`afterAll` cleanup for the test's env-var clearing, per CodeRabbit's nit about cross-suite env leakage.
Verified: 548/548 backend tests pass, `tsc --noEmit` clean.
**PR #58 remaining 15 D-008-era review findings, fixed (2026-07-31)**:
- `api.ts`: `/agent-room/messages`'s `limit` query param now parsed with `Number()` + `Number.isInteger()` instead of `parseInt()`, rejecting partially-numeric input like `"20abc"` instead of silently truncating it to `20`. `/repo/:name/audit` now validates `req.params['name']` against the same allowlist regex the other `/repo/:name` routes already use, and validates *before* the `200 OK` ack (previously acknowledged, then validated after — an invalid name could reach the audit flow while still being reported as queued).
- `securityDb.ts`/`types/securityRow.ts`: added a dedicated `SecurityScoreSummaryRow` type matching `getPortfolioSecuritySummary()`'s actual `DISTINCT ON` projection (no `id`/`recorded_at` — those aren't selected), replacing the misleading reuse of the full `SecurityScoreRow`. Updated the 2 callers (`ceoReport.ts`, `telegramCommands.ts`) that explicitly annotated the old type.
- `portfolioDb.ts`: `upsertRepoMetrics()` now defaults `priority`/`builds_passed`/`builds_failed`/`tasks_done`/`tasks_queued`/`debugger_runs` in application code (`?? 'medium'` / `?? 0`) instead of `?? null` — a real bug, not just a type mismatch: an explicit `NULL` parameter overrides a column's `DEFAULT`, so omitted values were persisting as `NULL` instead of ever falling back to the DDL's `'medium'`/`0` defaults.
- `selfAuditor.ts`: the `!auditCycle` early-return path now marks the self-audit cycle `failed` before returning, instead of leaving it stuck `active` when cycle creation fails.
- `sentinelBrain.ts`: `recordBrainOutcome()`'s `avgDelta` is now normalized to a number at write time (`Number((total/count).toFixed(2))` instead of `.toFixed(2)`), fixing a `string | number` union that contradicted `BrainHistoryRow`'s declared `number` type. `validateBrainOutput()` (`aiOutputValidator.ts`) now normalizes malformed `alerts`/`skip_repos` (e.g. a bare string instead of an array) to `[]` instead of letting them through untyped — previously a model response shaped that way would pass validation, get auto-executed and persisted, then crash `.forEach()` downstream. Fixes both the `sentinelBrain.ts` and `types/brainDecision.ts` findings (same root cause).
- `auditDb.ts`: `createAuditTask()` now defaults `priority ?? 'medium'`, `complexity ?? 'medium'`, `safeToAutoExecute ?? false` before insert, matching `AuditTaskRow`'s non-null contract instead of letting node-postgres silently convert omitted values to `NULL`.
- `workers/dailyReportWorker.ts`: the stale-tasks `COUNT(*)` query result is now typed `count: string` (matching PostgreSQL's `bigint` return and the same pattern already used in `sentinelBrain.ts`), parsed via `Number.parseInt()` at the point of use instead of being declared `number` when node-postgres actually returns a string.
- `notionDashboard.ts` + `weeklyBusinessReport.ts`: both business-metric reporting paths now check `metric_value != null` before formatting, showing `N/A` (dashboard) or omitting the line (weekly report) instead of reporting a null metric as a false `$0`/`0`.
- `sprintOrchestrator.ts`: the `completed_tasks`/`failed_tasks` counter updates and `completeSprint()` now explicitly check for a missing sprint row (`getSprintById` returning `null`) and log-and-abort/skip instead of silently defaulting to `0`/`'unknown'` — a missing row now surfaces loudly instead of masking a potential lost-aggregate-update bug.
- `types/businessMetricRow.ts`: `recorded_date` retyped from `string` to `Date` — no `pg` type-parser override is registered for the `DATE` OID anywhere in this codebase, so node-postgres's default parser (which returns a `Date`) actually applies; confirmed no caller currently calls a string-only method on this field, so this was a silent type/runtime mismatch rather than an active bug.
- `types/taskBuilder.ts`/`taskBuilder.ts`/`claudeCodeRunner.ts`: both `buildAiderTaskMessage()` and `buildTaskPrompt()` now normalize `task.description` (`?? ''`) and `task.category` (`?? 'general'`) before interpolating them into the AI prompt sent to the builder agent — previously a task with a null description/category would literally render the string `"null"` or `"undefined"` in the prompt.
- `velocityTracker.ts`: `getVelocityReport()` now parses/defaults the nullable `avg_health` (`parseFloat(... || '0').toFixed(1)`) for both the trend-line list and the "Latest" summary line, instead of interpolating a possibly-null value directly (which would have rendered as `health null/10`).
Verified: 548/548 backend tests pass, `tsc --noEmit` clean.
**Status**: All 21 original PR #58 review findings (5 ai-client + 16 D-008-era, one of which CodeRabbit auto-resolved as a duplicate) are now fixed.
...
**Scope**: `backend/src/obsidianLedgerWriter.ts` (2026-09-16) mirrors every `audit_tasks` create/status-update into `OBSIDIAN-TEAM-BOARDROOM/ledger/tasks/<NNN>-*.md` + a matching `ledger/pool.md` row. This is a deliberate, explicitly-approved exception to that repo's own stated rule in `scripts/sentinel_bridge.py` ("no cross-repo writes from Sentinel, so branch isolation is preserved") — Sentinel now does write cross-repo for this one purpose.
**Decision (2026-09-16, Shayan)**: Direct push to `master`, no PR — chosen over both alternatives (Sentinel opens a PR per change; a scheduled batch sync) for speed, since Cloudflare Cockpit's live data depends on this ledger being current. This is a real, explicit exception to `OBSIDIAN-TEAM-BOARDROOM`'s own Rule 2 ("never commit... directly to `main`") — documented there as "Automated Ledger Mirror Exception" in that repo's `REPO_RULES.md`, not a silent bypass. Confirmed that repo's `master` has no GitHub-level branch protection (private repo, no GitHub Pro) — this exception is the *only* thing standing between a writer bug and a broken ledger, so the implementation deliberately narrows its blast radius:
- Gated behind `OBSIDIAN_BOARDROOM_WRITE_TOKEN` (a separate, fine-grained PAT scoped only to `Thatisshayan/OBSIDIAN-TEAM-BOARDROOM`, distinct from the broader `GITHUB_TOKEN` this backend uses elsewhere for cloning/pushing to audited repos). Unset by default — dev machines keep the original sibling-checkout-only, write-but-never-push behavior with zero config changes.
- When set, the writer no longer depends on (or even looks at) `OBSIDIAN_BOARDROOM_PATH` — it clones `OBSIDIAN-TEAM-BOARDROOM` fresh into a temp directory on every single call, so it always operates on the true remote state (this also fixes the writer being a complete no-op in production, since no sibling checkout exists there) and so `.sentinel-task-map.json` survives across otherwise-stateless runs.
- The push only ever stages `ledger/tasks/`, `ledger/pool.md`, and `ledger/.sentinel-task-map.json` — never `git add -A` — so a bug here cannot carry unrelated working-tree state to `master`.
- Commits are clearly machine-tagged (`sentinel-ledger-writer` git identity, `[automated]` commit-message suffix) for auditability.
**Also not done in this pass** (unchanged, still open):
- No lock/atomicity between this writer's `nextLedgerNumber()` scan and `scripts/sentinel_bridge.py`'s own `next_id()` scan — both compute "max existing `ledger/tasks/NNN-*.md` + 1" independently. A collision is only possible if both run at nearly the exact same instant; defense-in-depth retry exists (bump + recheck) but no true lock.
**Status**: Resolved — push-to-master implemented and tested (`backend/test/obsidianLedgerWriter.test.ts`, 8/8 passing including 3 new push-mode cases: real push, idempotent no-op when nothing changed, and temp-clone cleanup on push failure). Companion PR in `OBSIDIAN-TEAM-BOARDROOM` documents the exception in `REPO_RULES.md`. Still needs, before this is live: the real `OBSIDIAN_BOARDROOM_WRITE_TOKEN` generated and set as a deployment secret (not yet done — this pass only ships the code path).
