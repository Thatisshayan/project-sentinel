...
**Deferred (per-caller shape too different for a naive migration)**: `claudeCodeAudit.ts`/`owaspChecker.ts`/`agentRoom.ts` — fixed-endpoint or structurally different callers; not touched.
**Verified**: 544/544 backend tests pass (including new `test/ai.client.test.ts` covering the fallback loop, model overrides, and configured/unconfigured error paths), `tsc --noEmit` clean.
**Status**: Partially complete — 4/7 original call sites migrated. Remaining: `claudeCodeAudit`/`owaspChecker`/`agentRoom` still deferred, same reasoning as before.
...
**Remaining**: `commands/repoOps.ts` (still ~62 `any`s, mostly `catch`/inline-`require()` casts), `workers/dailyReportWorker.ts`, `sentinelBrain.ts`, `notionClient.ts`, `telegramAI.ts`, and the other ~85 files not yet surveyed.
**Status**: Partially complete — `api.ts` Express typing + the agent-row type chain done. Prefer per-file `noImplicitAny` tightening over a repo-wide flag flip for the rest, to keep each PR reviewable and testable independently.
