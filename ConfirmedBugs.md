...
**Verification for pass 3, round 1:** all fixes pushed to PR #33; re-ran full backend test suite plus updated `webhook.test.js` (claim/release semantics), `telegramAI.pickSpeakingAgent.test.ts` (negative over-match cases) — see the PR's CI run for the actual pass/fail count, not asserted here from memory. `npx tsc --noEmit` re-run clean.

CodeRabbit re-reviewed the round-1 fixes above and found 3 more real issues, applied the same day in a second commit on PR #33 (this section was missed when pass 3 was first written up — added retroactively so this document isn't silently incomplete):

26. **`auditOrchestrator.ts` `checkApprovalTimeout` had a TOCTOU race** — a separate `SELECT` then `UPDATE` meant a human approval landing between the two could get overwritten back to `'skipped'` by the timeout firing right after. Replaced with a single atomic `UPDATE ... WHERE status = 'awaiting_approval' RETURNING id` — it only ever affects a cycle still genuinely pending at write time. Also stopped swallowing DB/state-transition errors in the catch block: this runs inside a BullMQ job handler, so letting a genuine failure throw lets BullMQ's own retry/backoff actually retry instead of silently leaving the cycle stuck forever (notification failure alone is still non-fatal).
27. **`auditOrchestrator.ts` / `sprintOrchestrator.ts` — `enqueueScheduledJob` failures (approval-timeout scheduling, sprint continuation) were only logged as a warning**, with nothing to notice or recover. A full retry/recovery workflow was flagged by the reviewer as genuinely heavy-lift and out of scope; the proportionate fix applied instead was a visible Telegram alert on failure, so a human knows to intervene manually instead of the cycle/sprint silently stalling with zero signal.
28. **`webhook/processWebhook.ts` — the claim-then-release fix (bug 24) released the dedup claim on ANY Notion failure**, including permanent ones (bad auth, wrong database ID) that a redelivery retry can never fix — those would just fail identically forever, burning retries for nothing. Added a permanent-vs-retryable classification keyed on Notion's API error codes (`unauthorized`/`restricted_resource`/`object_not_found` = permanent, keep the claim; anything else, including no `.code` at all, = retryable, release it).

**Still not verified after pass 3 (either round):** none of these fixes were manually smoke-tested the way bug 16 was in the pass-2 update above — they're covered by unit tests and `tsc`, not a live click-through. The `SPRINT_CONTINUE_JOB` fix (17) in particular can only be truly proven by watching a real multi-task sprint execute against a real Redis, which hasn't happened.

---

## Pass 4 — 2026-07-19: user-requested feature (richer audit report) + a self-identified footgun

Two more real issues, both fixed same day on a new PR (#34), reviewed by CodeRabbit, merged as commit `8eefe60`:

29. **`dbClient.ts` `updateDebugAttempt` — the dynamic-column-name footgun flagged (but not fixed) back in the original scan finally got tackled.** It built its `SET` clause straight from `Object.keys(updates)` with no allowlist; safe only because every real call site passes hardcoded literal keys, but the next caller passing a dynamic/user-influenced key would have been a SQL injection point via the column identifier itself (column names can't be parameterized the way values can). Added an explicit allowlist of the table's actual mutable columns (`attempt_number`, `max_attempts`, `status`, `debugger_used`, `fix_commit_sha`, `fix_commit_url`, `fix_branch`, `fix_pr_url`, `failure_reason`, `build_provider`, `build_url`, `high_risk`, `high_risk_reason`) — a non-allowlisted key is now dropped and logged as an error instead of reaching SQL.
30. **`auditOrchestrator.ts` — the newly-expanded audit report (see feature note below) could silently vanish for repos with many/long-reasoned tasks.** `sendMenu()` (the inline-approval-button send path) swallows its own axios errors internally and never throws, so the existing `try { sendMenu(...) } catch { fallback to sendTelegramMessage }` could never actually reach its fallback — an oversized message wouldn't fail loudly, it would silently no-op with only a warn-level log, and the operator would get nothing. CodeRabbit caught this on PR #34's review. Fixed by truncating the message to Telegram's real 4096-char limit before it's ever handed to `sendMenu`, mirroring the truncation `sendTelegramMessage`'s own fallback already does.

**Feature, not a bug fix:** the audit-complete Telegram message was also expanded per a direct user request — it now shows a priority breakdown (🔴/🟠/🟡/🟢 counts), each task's category, the audit's actual safety reason for locked/needs-review tasks (previously just a bare 🔒 with no context), and a health-score trend vs. the repo's last audit (↑/↓/→ with the delta) via a new `auditDb.getPreviousHealthScore()`. This is what made bug 30 above easy to trigger — longer messages are more likely to cross the 4096-char threshold — hence fixing both in the same PR.

**Verification for pass 4:** `npm test` → 35 suites / 266 tests passing; `npx tsc --noEmit` clean. New tests: `dbClient.updateDebugAttempt.test.ts` (allowlist enforcement, injection-shaped key rejection, all-keys-rejected no-op) and additions to `auditOrchestrator.test.js` (trend shown/omitted/negative, priority breakdown ordering, category tags, lock-reason text). **Not live-verified** — no real audit has actually run against this code in production yet; the richer message format has not been visually confirmed in a real Telegram client.

---

## Ops log (not code bugs — live production actions taken 2026-07-19)

Separate from the bug-fix passes above: while testing the merged fixes against the real Railway deployment, two live-infrastructure actions were taken directly against production, at the user's explicit direction each time:

- **Cleared 13 stuck `audit_cycles` rows** (3 `awaiting_approval`, 10 `executing`) that predated every fix in this document — dated between 2026-06-13 and 2026-07-10, i.e. 9 days to over a month old, well past any legitimate audit/approval window. These existed because the `setTimeout`-based approval timeout and sprint-continuation code (bugs #6, 12, 13 above) had no persistence before today's fixes, so a Railway redeploy at any point during that window would have permanently orphaned whatever cycle was in flight. All 13 were set to `status = 'skipped'` via a direct `UPDATE` through `railway connect postgres` (run by the user, not by the agent — direct production DB writes are blocked for the agent by this environment's safety classifier, by design). Confirmed via `/health`: `awaiting_approval` and `executing` both read `0` immediately after.
- **Disconnected the Vercel Git integration** for the `project-sentinel` Vercel project. That project had zero environment variables configured (`SENTINEL_API_URL`/`SENTINEL_UI_KEY` both unset) and was auto-deploying a non-functional copy of the dashboard on every push (confirmed live: `/api/stats` returned `{"error":"no backend"}`) — Railway is the actual, correctly-configured production UI. Disconnected via `vercel git disconnect`; the Vercel project itself still exists, it just no longer redeploys from GitHub. A small side effect (`ui/.gitignore` gaining a `.env*` entry, from `vercel link`'s local setup) was committed separately (commit `0bc867b`).

Neither of these is "done" in the sense of a permanent fix — the Vercel project could be reconnected by anyone with access, and nothing currently *alerts* if audit cycles start piling up again before their timeout fires (a recommendation, not yet built).

---

## Pass 5 — 2026-07-19: findings from a live-audit test that failed on 3 different repos

Attempting to trigger a real audit end-to-end (to visually confirm the Pass 4 richer Telegram report) surfaced three real, distinct issues — none are bugs in the report feature itself:

31. **`api.ts` manual-audit routes hardcoded `branchName: 'main'`.** `/repo/:name/audit` and `/system/audit-all` both passed a literal `'main'` into `triggerAudit()` regardless of the repo's actual default branch. Confirmed live: triggering an audit on `let-it-rain` (whose default branch is not `main`) failed with `fatal: Remote branch main not found in upstream origin`. Fixed by adding `repoDiscovery.getDefaultBranch(repoFullName)` — a small GitHub API lookup (`GET /repos/{full_name}`, falls back to `'main'` on any error or missing token) — and using its result instead of the hardcoded literal in both routes.
32. **`agents-ops-board` (in the tracked repo list) points at a nonexistent GitHub repository.** Every audit attempt against it fails with `remote: Repository not found` — confirmed for 3 separate cycles (this session's, plus prior cycles 74 on 2026-07-06 and 65 on 2026-06-29 via `railway logs`). Not fixed — the tracked-repo list itself needs correcting (rename, re-point, or remove), which needs the user to say what the repo is actually called now / whether it still exists.
33. **Near-portfolio-wide `audit_tasks` backlog silently blocks Rule 2.** Querying `audit_tasks` across all tracked repos found 10–25 unresolved `queued` tasks in nearly every one (`alphonso`, `AlphonsoEcosystem`, `costpilot`, `mint`/`Mint`, `obsidian-media`, `project-sentinel`/`projectSentinel`, `Safircountertop`, `session-guard`, `shiporex`, `tapcash`/`Tapcash`) — well over Rule 2's threshold of 3. This means `checkAuditRules`' Rule 2 (skip audit if queued tasks ≥ threshold) is silently blocking new audits on most of the portfolio, and has been for a while. Not fixed — this is a backlog/approval-workflow problem, not a code bug; see TODO 7.9 for the related "alert before things go stale" recommendation.

**Verification:** `npx tsc --noEmit` clean; `npm test` → 36 suites / 270 tests passing. **Not live-verified** — bug 31's fix has not yet been tested against a real repo with a non-`main` default branch in production; findings 32 and 33 are data/config issues, not shippable code fixes, and remain open.

**CodeRabbit review (PR #35), applied same day:**
34. **`/system/audit-all` serialized what used to be a fire-and-forget parallel dispatch.** My first version of the bug-31 fix used a `for...await` loop, awaiting `getDefaultBranch()` (up to a 10s timeout) before triggering each repo's audit — meaning a bulk audit across N repos could take up to N×10s to finish *scheduling*, instead of firing near-instantly like before. Fixed by resolving each repo's branch and dispatching `triggerAudit` independently (`.then()` per repo, not awaited in a loop) so all repos are scheduled concurrently again; a slow/failed branch lookup for one repo no longer delays or blocks the others.
35. Nitpick (addressed): added `repoDiscovery.getDefaultBranch.test.ts` — 4 tests covering a successful non-`main` response, missing-`GITHUB_TOKEN` fallback (no request made), request-failure fallback, and missing-`default_branch`-field fallback.
