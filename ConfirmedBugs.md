...
**Verification:** `npx tsc --noEmit` clean; `npm test` → 35 suites / 266 tests passing (no test changes needed — the fix only changes what value is threaded into an existing parameter, and no test previously exercised routes 31 covers). **Not live-verified** — bug 31's fix has not yet been tested against a real repo with a non-`main` default branch in production; findings 32 and 33 are data/config issues, not shippable code fixes, and remain open.
