...
async function checkGitHubActions(repoFullName, commitSha) {
    try {
        ...
        const runsRes = await axios_1.default.get(`https://api.github.com/repos/${repoFullName}/actions/runs`, { headers, params: { head_sha: commitSha, per_page: 10 } });
        ...
        if (runs.length === 0) {
            try {
                const recentRes = await axios_1.default.get(`https://api.github.com/repos/${repoFullName}/actions/runs`, { headers, params: { per_page: 5 } });
                const fiveMinutesAgo = Date.now() - 5 * 60 * 1000;
                const latestRun = (recentRes.data.workflow_runs || []).find((r) => new Date(r.created_at).getTime() > fiveMinutesAgo);
                if (latestRun) {
                    runs = [latestRun];
                    logger_1.default.info({ repoFullName, runId: latestRun.id }, 'SHA lookup empty — using latest recent run as fallback');
                }
            }
            catch (e) {
                logger_1.default.warn({ err: e.message }, 'Branch fallback run lookup failed');
            }
        }
        ...
        if (runs.length === 0)
            return { provider: 'github_actions', status: 'pending' };
        ...
        if (status === 'failed') {
            try {
                const jobsRes = await axios_1.default.get(`https://api.github.com/repos/${repoFullName}/actions/runs/${activeRun.id}/jobs`, { headers });
                ...
                logger_1.default.warn({ err: e.message }, 'Could not fetch GitHub Actions job details');
            }
        }
        return {
        ...
        logger_1.default.warn({ err: err.message, repoFullName }, 'GitHub Actions check failed');
...
async function checkVercel(repoFullName, commitSha) {
    ...
    try {
        const res = await axios_1.default.get('https://api.vercel.com/v6/deployments', {
        ...
        logger_1.default.warn({ err: err.message }, 'Vercel check failed');
...
async function checkRailway(repoFullName, commitSha) {
    ...
    try {
        const res = await axios_1.default.post('https://backboard.railway.app/graphql/v2', {
            query: `
        ...
        let match = edges.find((e) => e.node.meta && e.node.meta.commitHash === commitSha);
        ...
        if (!match) {
            ...
            match = edges.find((e) => new Date(e.node.createdAt).getTime() > fifteenMinutesAgo &&
                ['SUCCESS', 'FAILED', 'CRASHED', 'BUILDING', 'DEPLOYING'].includes(e.node.status));
            if (match) {
                logger_1.default.info({ repoFullName, status: match.node.status }, 'Railway SHA match failed — using most recent deployment as fallback');
            }
        }
        if (!match)
            return { provider: 'railway', status: 'not_configured' };
        ...
        logger_1.default.warn({ err: err.message }, 'Railway check failed');
...
async function checkAllProviders(repoFullName, commitSha) {
    ...
    const results = [github, vercel, railway].filter(r => r.status !== 'not_configured');
    if (results.length === 0) {
