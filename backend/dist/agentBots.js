...
async function sendAsAgent(agentId, text, replyToMessageId = null) {
    ...
    if (!CHAT_ID()) {
        logger_1.default.warn({ agentId }, 'TELEGRAM_CHAT_ID not set — cannot send agent message');
        return null;
    }
            ...
            res.on('end', () => {
                try {
                    ...
                    if (parsed.ok) {
                        ...
                        logger_1.default.warn({ agentId, error: parsed.description }, 'Agent bot send failed — falling back');
