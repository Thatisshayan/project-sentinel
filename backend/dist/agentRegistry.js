...
async function initAgentPool() {
    ...
    for (const agent of AGENT_POOL) {
        const config = (0, builderRouter_1.getBuilderConfig)(agent.id);
        if (config.envKey && !process.env[config.envKey])
            continue;
        await (0, agentDb_1.registerAgent)(agent.id, config.label);
        registered++;
    }
    await (0, agentRoom_1.broadcastToAll)(`Agent pool initialised — ${registered} agents registered`);
    logger_1.default.info({ registered }, 'Agent pool initialised');
}
async function selectAgent(taskComplexity, preferredBuilder) {
    if (preferredBuilder) {
        const config = (0, builderRouter_1.getBuilderConfig)(preferredBuilder);
    ...
    for (const agentId of preferred) {
        ...
        if (currentCount < poolEntry.maxConcurrent) {
            const config = (0, builderRouter_1.getBuilderConfig)(agentId);
            if (!config.envKey || process.env[config.envKey]) {
                return agentId;
            }
        }
    ...
    for (const poolEntry of AGENT_POOL) {
        ...
        if (currentCount < poolEntry.maxConcurrent) {
            const config = (0, builderRouter_1.getBuilderConfig)(poolEntry.id);
            if (!config.envKey || process.env[config.envKey]) {
                return poolEntry.id;
            }
        }
    }
    logger_1.default.warn('No available agents — all at capacity');
    return AGENT_POOL[0].id;
}
async function assignAgent(agentId, taskData) {
    await (0, agentDb_1.setAgentWorking)(agentId, taskData);
}
async function freeAgent(agentId, success = true) {
    await (0, agentDb_1.setAgentIdle)(agentId, success);
}
module.exports = {
    initAgentPool,
    selectAgent,
