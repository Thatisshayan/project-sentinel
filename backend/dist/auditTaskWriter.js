...
async function writeTasksToNotion(auditResult, auditCycleId, payload) {
    const { repoFullName, projectName, repoName, commitSha, notionParentPageId, builderAgent, } = payload;
    const written = [];
    const skipped = [];
    const failed = [];
    ...
    for (const task of auditResult.tasks) {
        ...
        const isDuplicate = await (0, auditDb_1.checkDuplicateTask)(repoFullName, task.title);
        if (isDuplicate) {
            skipped.push({ taskNumber: task.taskNumber, title: task.title });
            logger_1.default.info({ title: task.title }, 'Skipping duplicate task');
            continue;
        }
        ...
        try {
            ...
            logger_1.default.warn({ taskNumber: task.taskNumber, err: err.message }, 'Failed to write task to Notion — continuing');
            failed.push({
                taskNumber: task.taskNumber, title: task.title, reason: err.message,
            });
        }
        try {
            await (0, auditDb_1.createAuditTask)({
                auditCycleId,
                repoFullName,
            ...
            logger_1.default.error({ taskNumber: task.taskNumber, err: err.message }, 'Failed to save task to database');
        }
    }
...
async function updateNotionTaskStatus(notionPageId, status, extra = {}) {
    ...
    try {
        ...
        if (extra.prUrl)
            props['PR URL'] = { url: extra.prUrl };
        if (extra.commitUrl)
            props['Commit URL'] = { url: extra.commitUrl };
        ...
        logger_1.default.warn({ err: err.message, notionPageId }, 'Could not update Notion task status');
    }
}
module.exports = { writeTasksToNotion, updateNotionTaskStatus };
//# sourceMappingURL=auditTaskWriter.js.map