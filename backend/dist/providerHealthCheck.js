...
async function probeAIProviders() {
    ...
    for (const p of probes) {
        ...
        try {
            await axios_1.default.get(p.url, {
                headers: { Authorization: p.auth() },
                timeout: 6000,
            });
            results.push(`  ✓ ${p.name}: reachable`);
            logger_1.default.info({ provider: p.name }, 'AI provider reachable');
        }
        catch (err) {
            ...
            if (status === 401 || status === 403) {
                ...
                logger_1.default.info({ provider: p.name }, 'AI provider 400 — key likely valid, /models unsupported');
            }
            else {
                results.push(`  ? ${p.name}: ${status || err.code || err.message}`);
            }
            logger_1.default.warn({ provider: p.name, status }, 'AI provider probe failed');
        }
    }
    logger_1.default.info({ results }, 'AI provider health check complete');
    // Alert if any keys are definitely invalid (401/403)
    const invalidProviders = results.filter(r => r.includes('✗'));
    if (invalidProviders.length > 0) {
        await (0, safeFire_1.safeFire)((0, telegramClient_1.sendTelegramMessage)(`🔴 Sentinel AI Provider Alert\n\n` +
            `${invalidProviders.length} provider(s) have invalid API keys:\n` +
            `${invalidProviders.join('\n')}\n\n` +
            `Go to Railway → Variables and update the key(s) to restore those agents.`, null, null), { label: 'providerHealthCheck' });
        ...
        for (const line of invalidProviders) {
            ...
            for (const agentId of (PROVIDER_AGENT_MAP[provider || ''] || [])) {
                await (0, safeFire_1.safeFire)((0, agentDb_1.markAgentError)(agentId, 'invalid_api_key'), { label: 'providerHealthCheck' });
            }
        }
    }
    return results;
...
//# sourceMappingURL=providerHealthCheck.js.map