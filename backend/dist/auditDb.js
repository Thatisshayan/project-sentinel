...
async function initAuditSchema() {
  ...
  `);
    logger_1.default.info('Audit schema initialised');
}
async function getAuditCycle(repoFullName, commitSha) {
    const r = await query('SELECT * FROM audit_cycles WHERE repo_full_name=$1 AND commit_sha=$2', [repoFullName, commitSha]);
...
async function updateAuditCycle(id, updates) {
    ...
    const r = await query(`UPDATE audit_cycles SET ${fields}, updated_at=NOW() WHERE id=$1 RETURNING *`, [id, ...values]);
    return r.rows[0] || null;
}
...
async function updateAuditTask(id, updates) {
    ...
    const values = Object.values(updates);
    const fields = keys.map((k, i) => `${k} = $${i + 2}`).join(', ');
    const r = await query(`UPDATE audit_tasks SET ${fields}, updated_at=NOW() WHERE id=$1 RETURNING *`, [id, ...values]);
    return r.rows[0] || null;
}
...
//# sourceMappingURL=auditDb.js.map