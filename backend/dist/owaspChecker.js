...
Be conservative. Do not invent issues.`;
    ...
    try {
        ...
        logger_1.default.warn({ err: err.message, repoName }, 'OWASP parse failed — using unknowns');
        evaluation = OWASP_ITEMS.map(item => ({
            id: item.id, status: 'unknown', notes: 'Evaluation failed',
        }));
    }
    for (const item of evaluation) {
        await (0, safeFire_1.safeFire)((0, securityDb_1.upsertOwaspItem)(repoName, item.id, item.status, item.notes), { label: 'owaspChecker' });
    }
    ...
    logger_1.default.info({ repoName, owaspScore }, 'OWASP evaluation complete');
    return { results: evaluation, owaspScore };
}
module.exports = { evaluateOwasp, OWASP_ITEMS };
//# sourceMappingURL=owaspChecker.js.map