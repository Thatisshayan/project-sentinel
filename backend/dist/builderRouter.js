...
function getBuilderConfig(assignment) {
    ...
    if (!config) {
        logger_1.default.warn({ assignment, fallback: DEFAULT_BUILDER }, 'Unknown builder assignment — using default');
    ...
    if (config.envKey && !process.env[config.envKey]) {
        ...
        for (const candidate of chain) {
            ...
            if (fb && (!fb.envKey || process.env[fb.envKey])) {
                logger_1.default.warn({ builder: key, envKey: config.envKey, fallback: candidate }, 'Builder API key missing — falling back');
                return fb;
            }
        }
        ...
        logger_1.default.warn({ builder: key, envKey: config.envKey }, 'Builder API key missing and no fallback available');
        return config;
    }
    return config;
}
...
//# sourceMappingURL=builderRouter.js.map