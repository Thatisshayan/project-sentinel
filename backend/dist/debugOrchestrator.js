...
const aiderRunner_1 = require("./aiderRunner");
...
async function orchestrateDebug(payload) {
    const { projectName, repoName, repoFullName, branchName, commitSha, commitUrl, commitMessage, authorName, changedFiles, buildProvider, buildUrl, logsUrl, failureReason, failureLogs, topicId, } = payload;
    logger_1.default.info({ repoFullName, commitSha: commitSha?.slice(0, 7) }, 'Debug orchestration started');
    const existing = await (0, dbClient_1.getDebugAttempt)(repoFullName, commitSha);
    if (existing && existing.status === 'stopped') {
        logger_1.default.info({ repoFullName }, 'Debug attempts stopped by human — skipping');
        return;
    }
    const logRisk = (0, riskAssessor_1.assessLogRisk)((0, riskAssessor_1.sanitizeLogs)(failureLogs), buildProvider);
    ...
    if (logRisk.isHighRisk || fileRisk) {
        ...
        await (0, telegramClient_1.sendTelegramMessage)(buildHighRiskMessage({ projectName, repoName, branchName, commitSha,
            buildProvider, buildUrl, failureReason, reason }), null, topicId);
        await updateNotionForHighRisk(repoFullName, commitSha, {
            failureReason, buildProvider, buildUrl, reason,
        });
        logger_1.default.warn({ repoFullName, reason }, 'High-risk failure — debug blocked');
        return;
    }
    let attempt = existing;
    const max = MAX_ATTEMPTS();
    if (!attempt) {
        attempt = await (0, dbClient_1.createDebugAttempt)({
            repoFullName, commitSha, buildProvider, buildUrl,
            failureReason: (0, riskAssessor_1.sanitizeLogs)(failureReason || '').substring(0, 500),
        });
    }
    if (!attempt) {
        logger_1.default.error({ repoFullName }, 'Could not create debug attempt record');
        return;
    }
    if (attempt.attempt_number >= max) {
        await (0, telegramClient_1.sendTelegramMessage)(buildExhaustedMessage({ projectName, repoName, branchName, commitUrl,
            attempt, buildUrl }), null, topicId);
        await (0, dbClient_1.updateDebugAttempt)(repoFullName, commitSha, { status: 'exhausted' });
    ...
    await (0, dbClient_1.incrementAttempt)(repoFullName, commitSha, agent);
    await updateNotionState(repoFullName, 'Debugging');
    await (0, telegramClient_1.sendTelegramMessage)(buildStartingMessage({ projectName, repoName, attemptNumber: nextAttemptNum,
        max, agentLabel }), null, topicId);
    if (DRY_RUN()) {
        logger_1.default.info({ repoFullName, attemptNumber: nextAttemptNum }, 'DRY RUN — no changes made');
        await (0, telegramClient_1.sendTelegramMessage)(buildDryRunMessage({ projectName, repoName, attemptNumber: nextAttemptNum,
            agentLabel, failureReason: (0, riskAssessor_1.sanitizeLogs)(failureReason || '') }), null, topicId);
        await (0, dbClient_1.updateDebugAttempt)(repoFullName, commitSha, { status: 'dry_run' });
        return;
    }
    const fixContext = {
        projectName, repoName, repoFullName, branchName,
        commitSha, changedFiles, buildProvider, buildUrl,
        failureReason: (0, riskAssessor_1.sanitizeLogs)(failureReason || ''),
        failureLogs: (0, riskAssessor_1.sanitizeLogs)(failureLogs || ''),
        attemptNumber: nextAttemptNum,
        agentLabel,
    };
    let fixResult;
    try {
        fixResult = await (0, aiderRunner_1.cloneAndFix)(fixContext);
    }
    catch (err) {
        logger_1.default.error({ err: err.stack ?? err.message }, 'cloneAndFix threw unexpectedly');
    ...
    if (fixResult.status === 'fixed') {
        ...
        await (0, dbClient_1.updateDebugAttempt)(repoFullName, commitSha, {
        ...
        await (0, telegramClient_1.sendTelegramMessage)(buildFixReadyMessage({
        ...
        await (0, telegramClient_1.sendTelegramMessage)(buildCannotFixMessage({
            projectName, repoName, attemptNumber: nextAttemptNum,
...
async function updateNotionForHighRisk(repoFullName, commitSha, data) {
    try {
        ...
        logger_1.default.warn({ err: err.message }, 'Could not update Notion for high-risk failure');
    }
}
async function updateNotionState(repoFullName, state, extra = {}) {
    try {
        ...
        logger_1.default.warn({ err: err.message, state }, 'Could not update Notion state');
    }
}
...
//# sourceMappingURL=debugOrchestrator.js.map