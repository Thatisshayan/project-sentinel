...
async function trackAuditCost(repoFullName, promptLength, outputLength) {
    ...
    await (0, portfolioDb_1.logApiCost)({ repoFullName, operation: 'audit', model,
        inputTokens, outputTokens, estimatedCost: cost });
    logger_1.default.debug({ repoFullName, cost: cost.toFixed(4) }, 'Audit cost logged');
    return cost;
}
async function trackBuildTaskCost(repoFullName, promptLength, outputLength) {
    ...
    const outputTokens = estimateTokens(outputLength);
    const cost = estimateCost(model, inputTokens, outputTokens);
    await (0, portfolioDb_1.logApiCost)({ repoFullName, operation: 'build_task', model,
        inputTokens, outputTokens, estimatedCost: cost });
    return cost;
}
async function trackChatCost(promptLength, outputLength) {
    ...
    const outputTokens = estimateTokens(outputLength);
    const cost = estimateCost(model, inputTokens, outputTokens);
    await (0, portfolioDb_1.logApiCost)({ repoFullName: undefined, operation: 'chat_response', model,
        inputTokens, outputTokens, estimatedCost: cost });
    return cost;
}
async function getCostReport() {
    const [daily, monthly, byRepo] = await Promise.all([
        ...
        (0, portfolioDb_1.getCostByRepo)(7),
    ]);
    const repoLines = byRepo.slice(0, 8).map((r) => `  ${r.repo_full_name?.split('/')[1] || 'unknown'}: $${parseFloat(r.total).toFixed(3)} (${r.operations} ops)`).join('\n');
    return {
