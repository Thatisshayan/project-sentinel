...
async function scanDiff(diffText, repoFullName, scanId, commitSha) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (const pattern of SECRET_PATTERNS) {
            if (content.match(pattern.regex)) {
                ...
                await (0, safeFire_1.safeFire)((0, securityDb_1.insertSecurityIssue)(issue), { label: 'secretScanner' });
                issues.push(issue);
            }
        }
    ...
    logger_1.default.info({ repoFullName, secrets: issues.length }, 'Secret scan complete');
    return issues;
}
