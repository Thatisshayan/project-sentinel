...
async function initSprintSchema() {
  ...
  `);
    logger_1.default.info('Sprint schema initialised');
}
// ── Sprint helpers ────────────────────────────────────────────────────────────
...
async function updateSprint(id, updates) {
    ...
    const r = await query(`UPDATE sprints SET ${fields} WHERE id=$1 RETURNING *`, [id, ...values]);
    return r.rows[0] || null;
}
...
async function updateSprintTask(id, updates) {
    ...
    const values = Object.values(updates);
    const fields = keys.map((k, i) => `${k} = $${i + 2}`).join(', ');
    const r = await query(`UPDATE sprint_tasks SET ${fields}, updated_at=NOW() WHERE id=$1 RETURNING *`, [id, ...values]);
    return r.rows[0] || null;
}
...
//# sourceMappingURL=sprintDb.js.map