...
async function handleReportsCmd(subcommand, parts, chatId, topicId) {
    switch (subcommand) {
        ...
        case 'costs': {
            ...
            await (0, telegramClient_1.sendTelegramMessage)(report.formatted, null, topicId);
            return true;
        }
        case 'patterns': {
            ...
            if (patterns.length === 0) {
                await (0, telegramClient_1.sendTelegramMessage)('No cross-repo patterns detected.', null, topicId);
            }
            else {
                const lines = patterns.map((p) => `· ${p.description}\n  Repos: ${(p.affected_repos || []).join(', ')}`).join('\n\n');
                await (0, telegramClient_1.sendTelegramMessage)(`Cross-Repo Patterns:\n\n${lines}`, null, topicId);
            }
            return true;
        }
        case 'dashboard': {
            ...
            const [metrics, capacity, dailyCost, monthlyCost, activeAgents, queuedCount] = await Promise.all([
                getAllLatestMetrics().catch(() => []),
                getCapacityStatus().catch(() => ({})),
                getDailyCost().catch(() => 0),
                getMonthlyCost().catch(() => 0),
                (0, agentDb_1.getAllAgents)().catch(() => []),
            ...
            (0, safeFire_1.fireAndForget)(updateDashboard(), { label: 'reports' });
            await (0, telegramClient_1.sendTelegramMessage)(card, null, topicId);
            return true;
        }
        case 'velocity': {
            const { getVelocityReport } = require('../velocityTracker');
            const report = await getVelocityReport();
            await (0, telegramClient_1.sendTelegramMessage)(report, null, topicId);
            return true;
        }
        case 'performance': {
        ...
        case 'business': {
            ...
            if (parts[2]) {
                ...
                await (0, telegramClient_1.sendTelegramMessage)(summary || `No business metrics for ${parts[2]} yet.`, null, topicId);
            }
            else {
        ...
        case 'roi': {
            ...
            await (0, telegramClient_1.sendTelegramMessage)('ROI scores updated for all queued tasks.', null, topicId);
            return true;
        }
        case 'impact': {
            if (!parts[2]) {
                await (0, telegramClient_1.sendTelegramMessage)('Usage: /sentinel impact <repo-name>', null, topicId);
                return true;
            }
            const { getCorrelationSummary } = require('../correlationEngine');
            const corr = await getCorrelationSummary(parts[2]);
            if (!corr || !corr.pr_count) {
                await (0, telegramClient_1.sendTelegramMessage)(`No PR impact data for ${parts[2]} yet.`, null, topicId);
                return true;
            }
        ...
        case 'ceo': {
            ...
            await (0, telegramClient_1.sendTelegramMessage)('Generating CEO report...', null, topicId);
            generateCEOReport(topicId).catch((err) => logger_1.default.error({ err: err.stack ?? err.message }, 'Manual CEO report failed'));
            return true;
        }
        default:
            return false;
