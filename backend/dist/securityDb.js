...
async function initSecuritySchema() {
  ...
  `);
    logger_1.default.info('Security schema initialised');
}
...
async function updateSecurityScan(id, updates) {
    ...
    const r = await query(`UPDATE security_scans SET ${fields} WHERE id = $1 RETURNING *`, [id, ...values]);
...
async function getOpenIssues(repoFullName, severity = null) {
    ...
    END, found_at DESC`;
    if (severity) {
        const r = await query(`SELECT * FROM security_issues
       WHERE repo_full_name = $1 AND status = 'open' AND severity = $2 ${ORDER}`, [repoFullName, severity]);
        return r.rows;
    }
    const r = await query(`SELECT * FROM security_issues
     WHERE repo_full_name = $1 AND status = 'open' ${ORDER}`, [repoFullName]);
    return r.rows;
}
...
//# sourceMappingURL=securityDb.js.map