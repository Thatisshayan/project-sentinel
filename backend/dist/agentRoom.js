...
async function answerCallback(queryId, text = '') {
    if (!BOT_TOKEN())
        return;
    await (0, safeFire_1.safeFire)(telegramApiPost('answerCallbackQuery', {
...
async function generatePersonalityMessage(agentId, baseMessage) {
    ...
    if (!personality || !process.env['NVIDIA_API_KEY'])
        return baseMessage;
    try {
...
async function announce(agentId, agentLabel, message, type = 'info', repoName = null) {
    ...
    await (0, safeFire_1.safeFire)(sendAsAgent(agentId, fullMessage), { label: 'agentRoom' });
    await (0, safeFire_1.safeFire)((0, agentDb_1.logAgentMessage)(agentId, agentLabel, message, type, repoName ?? undefined), { label: 'agentRoom' });
    logger_1.default.info({ agentId, type, repoName }, message);
}
...
async function announceHandoff(fromAgentId, fromLabel, toLabel, repoName, context, extras = {}) {
    ...
    if (context)
        parts.push(context);
    if (completedTitle)
        parts.push(`Completed: "${completedTitle}"`);
    if (readyCount > 0)
        parts.push(`${readyCount} task(s) ready`);
    if (highRiskFiles.length)
        parts.push(`Watch: ${highRiskFiles.slice(0, 3).join(', ')}`);
    if (nextTask)
        parts.push(`First task: "${nextTask}"`);
...
async function sendConflictKeyboard(agentId, agentLabel, repoName, conflicts, conflictId) {
    if (!AGENT_ROOM_TOPIC_ID() || !BOT_TOKEN() || !CHAT_ID())
        return;
    ...
    await (0, safeFire_1.safeFire)(telegramApiPost('sendMessage', body), { label: 'agentRoom' });
}
...
async function buildStatusBoardText() {
    ...
    const queueR = await query(`SELECT COUNT(*) as count FROM audit_tasks WHERE status IN ('queued','in_progress')`).catch(() => ({ rows: [{ count: 0 }] }));
...
async function updatePinnedStatusBoard() {
    ...
    if (!text)
        return;
    const pinnedId = await (0, agentDb_1.getConfig)('pinned_status_message_id').catch(() => null);
    if (pinnedId) {
        await (0, safeFire_1.safeFire)(telegramApiPost('editMessageText', {
            chat_id: CHAT_ID(),
        ...
        const result = await (0, telegramClient_1.sendTelegramMessage)(text, null, AGENT_ROOM_TOPIC_ID() ? parseInt(AGENT_ROOM_TOPIC_ID(), 10) : null).catch(() => null);
        const messageId = result?.result?.message_id;
        if (messageId) {
            await (0, safeFire_1.safeFire)(telegramApiPost('pinChatMessage', {
                chat_id: CHAT_ID(),
                message_id: messageId,
            }), { label: 'agentRoom' });
            await (0, safeFire_1.safeFire)((0, agentDb_1.setConfig)('pinned_status_message_id', String(messageId)), { label: 'agentRoom' });
        }
    }
}
...
async function sendMorningBriefing() {
    ...
    try {
        ...
        const focus = summary?.metrics?.reduce((min, m) => (!min || m.health_score < min.health_score) ? m : min, null);
        if (focus)
            focusLine = `🎯 Focus: ${focus.repo_name} (health ${focus.health_score}/10)`;
    }
    catch { }
    const idle = agents.filter((a) => a.status === 'idle').length;
    const working = agents.filter((a) => a.status === 'working').length;
    const done24h = successRows.rows[0]?.count || 0;
    const queueLines = queueRows.rows.map((r) => `  · ${r.repo_full_name.split('/')[1]}: ${r.count} tasks`).join('\n');
    const msg = [
    ...
    await (0, safeFire_1.safeFire)((0, telegramClient_1.sendTelegramMessage)(msg, null, AGENT_ROOM_TOPIC_ID() ? parseInt(AGENT_ROOM_TOPIC_ID(), 10) : null), { label: 'agentRoom' });
}
...
//# sourceMappingURL=agentRoom.js.map