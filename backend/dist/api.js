...
router.use((req, res, next) => {
    ...
    if (key && (typeof headerKey !== 'string' || !(0, timingSafeCompare_1.timingSafeEqual)(headerKey, key))) {
...
router.get('/portfolio', async (req, res) => {
    ...
    `).catch(() => ({ rows: [] }));
        ...
        logger_1.default.error({ err: err.stack ?? err.message }, 'GET /portfolio error');
...
router.get('/repo/:name', async (req, res) => {
    ...
    `, [`%/${name}`]);
        ...
        logger_1.default.error({ err: err.stack ?? err.message }, 'GET /repo/:name error');
        res.status(500).json({ error: err.message });
    }
});
...
router.post('/command', async (req, res) => {
    ...
    await (0, safeFire_1.safeFire)(logAgentMessage('dashboard_user', fromName, text, 'info', null), { label: 'api' });
    ...
    handleCommand(text, null, null, fromName, null).catch((err) => logger_1.default.warn({ err: err.message }, 'Dashboard command failed'));
    res.json({ ok: true });
});
// ── Agent room messages ───────────────────────────────────────────────────────
...
router.get('/sprints/current', async (req, res) => {
    ...
    `);
        ...
        logger_1.default.error({ err: err.stack ?? err.message }, 'GET /sprints/current error');
        res.status(500).json({ error: err.message });
    }
});
...
router.post('/repo/:name/audit', async (req, res) => {
    ...
    try {
        ...
        const name = req.params.name;
        triggerAudit({
            repoFullName: (0, repoResolver_1.repoFullName)(name),
            repoName: name,
            projectName: name,
            ...
            authorEmail: '',
            topicId: null,
        }).catch((err) => logger_1.default.warn({ err: err.message, name }, 'Dashboard audit failed'));
    }
    catch (err) {
        logger_1.default.warn({ err: err.message }, 'Could not start dashboard audit');
    }
});
router.post('/system/audit-all', (req, res) => {
    ...
    for (const repo of REPO_LIST) {
        triggerAudit({
            ...
            authorEmail: '',
            topicId: null,
        }).catch((err) => logger_1.default.warn({ err: err.message, repo: repo.repoName }, 'Bulk audit item failed'));
    }
});
router.post('/system/security-scan', (req, res) => {
    ...
    for (const repo of REPO_LIST) {
        runSecurityScan({
            ...
            commitSha: 'HEAD',
            topicId: null,
        }).catch((err) => logger_1.default.warn({ err: err.message, repo: repo.repoName }, 'Bulk scan item failed'));
    }
});
...
router.get('/integrations/status', async (req, res) => {
    try {
        ...
        logger_1.default.error({ err: err.stack ?? err.message }, 'GET /integrations/status error');
        res.status(500).json({ error: err.message });
    }
});
// ── Settings ──────────────────────────────────────────────────────────────────
router.get('/settings', async (req, res) => {
    try {
        ...
        logger_1.default.error({ err: err.stack ?? err.message }, 'GET /settings error');
        res.status(500).json({ error: err.message });
    }
});
router.post('/settings/update', async (req, res) => {
    try {
        ...
        logger_1.default.error({ err: err.stack ?? err.message }, 'POST /settings/update error');
        res.status(500).json({ error: err.message });
    }
});
